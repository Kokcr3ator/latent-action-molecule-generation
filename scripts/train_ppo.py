...
def main() -> None:
    cfg = tyro.cli(Union[
        ...
        Annotated[FinetuneControllableCfg, tyro.conf.subcommand("finetune-controllable")],
    ])
    ...
    loader_cfg = SimpleNamespace(
        ckpt=SimpleNamespace(init_from="resume", path=cfg.pretrained_ckpt, ckpt_name="best.pt"),
        tokenizer=tok,
        ppo=cfg.ppo,
        reward=SimpleNamespace(task=cfg.task),
        context=SimpleNamespace(seq_len=tok.context_length),
        rl_train_cfg=SimpleNamespace(tokenizer_dir=tok_dir, ckpt_path=os.path.join(cfg.ckpt_root, run_name)),
    ...
    log.info(f"Environment: {env.num_envs} envs, action space [0, {env.action_space.upper})")
    ...
    for name, m in [("model", model), ("ppo_agent", ppo_agent)]:
        ...
        log.info("%s parameters: total=%s trainable=%s", name,
    ...
    hparams = HParams(
        ...
        num_envs=ppo.num_envs, num_steps=ppo.num_steps, budget=ppo.budget,
        num_epochs=ppo.num_epochs, num_minibatches=ppo.num_minibatches,
        clip_eps=ppo.clip_eps, ent_coef=ppo.ent_coef,
        max_episode_length=tok.context_length,
        vf_coef=ppo.vf_coef, max_grad_norm=ppo.max_grad_norm,
        lr=ppo.lr, normalise_advantage=ppo.normalise_advantage,
        clip_value_loss=ppo.clip_value_loss, gae_lambda=ppo.gae_lambda,
        discount=ppo.discount, weight_decay=ppo.weight_decay, anneal_lr=ppo.anneal_lr,
        lambda_kld=ppo.lambda_kld, log_frequency=ppo.log_frequency,
        log_to_wandb=cfg.wandb.enabled, wandb_project_name=cfg.wandb.project,
        save_dir=ckpt_path, eval_frequency=ppo.eval_frequency, random_start=ppo.random_start,
    )
    ppo_agent.hparams = hparams

    rl_train_cfg = RLTrainConfig(
        device=str(device), budget=ppo.budget,
        log_frequency=ppo.log_frequency, eval_frequency=ppo.eval_frequency,
        ckpt_path=ckpt_path, n_mols_generate=100, tokenizer_dir=tok_dir,
    ...
    train_dataloader = DataLoader(
        ...
        batch_size=2048, shuffle=False, drop_last=False, pin_memory=False,
    )

    trainer = RLTrainerBase(ppo_agent=ppo_agent, env=env, logger=logger, train_cfg=rl_train_cfg)
